...
class FacilitiesRepository(BaseRepository) : 
    ...
    async def set_room_facilities(self, room_id: int, facilities_ids: list[int]) :
        ...
        not_sorted_result = await self.session.execute(get_rooms_stmt) # вытаскиваем все строчки по номеру комнаты
        
        all_fclts_ids = {model.facility_id for model in not_sorted_result} # вытаскиваем все айдишники
        ids_to_delete = all_fclts_ids - set(facilities_ids) # вычисляем айдишники на удаление
        ids_to_add = set(facilities_ids) - all_fclts_ids # вычисляем айдишники на добавление
        ...
        if ids_to_add :
            ...
            add_bulk_stmt = (
                insert(self.model).values([item.model_dump() for item in data_to_add])
            )

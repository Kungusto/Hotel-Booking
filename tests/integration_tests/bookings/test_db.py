...
async def test_booking_crud(db: DBManager):
    await (
        db.bookings.delete()
    )  # удаляем предыдущие записи, т.к. они мешают тестировать crud-операции
    ...
    await db.bookings.edit(
        data_to_edit, is_patch=True, user_id=user_id, room_id=room_id
    )
    ...
    assert {
        ...
        "date_from": data_to_edit.date_from,
